...
document.querySelector("#btn-insert").onclick = () => {
  ...
  check()
  if(document.getElementById('m-password').value!=document.getElementById('m-password-chk').value) {
    alert("비밀번호가 같지 않습니다.");
    return
    }
  fetch("../members", {
...
function checkId(){
  ...
  const blankString = /[\s]/g;
  const specialPattern = /[`~!#$%^&*|\\\'\";:\/?]/gi;
  if (email.match(blankString)) {
    alert("공백이 포함되어있습니다");
    $('#email').val("");
    return
  }
  if (email.match(specialPattern)) {
...
function check_pw(){
 ...
 // 신규 배송접수 입력폼 -> 전화번호 : 숫자만 입력하면 자동으로 하이픈(-) 생성
...
function check() {
  ...
  if (chk.passwordChk.value == "") {
    alert("비밀번호 재입력을 입력해주십시오");
    chk.passwordChk.focus();
