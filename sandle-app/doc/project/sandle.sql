...
-- 산
DROP TABLE IF EXISTS hms_mountain_info RESTRICT;
...
-- 등산코스
DROP TABLE IF EXISTS hms_course_info RESTRICT;
...
-- 산
CREATE TABLE hms_mountain_info (
...
-- 산
ALTER TABLE hms_mountain_info
  ADD CONSTRAINT PK_hms_mountain_info -- 산 기본키
  PRIMARY KEY (
  mountain_info_id -- 산번호
  );

ALTER TABLE hms_mountain_info
  MODIFY COLUMN mountain_info_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '산번호';
...
-- 산악뉴스
CREATE TABLE hms_mountain_news (
  mountain_news_id INTEGER NOT NULL COMMENT '산악뉴스번호', -- 산악뉴스번호
  board_id         INTEGER NOT NULL COMMENT '게시판번호' -- 게시판번호
)
COMMENT '산악뉴스';

-- 산악뉴스
ALTER TABLE hms_mountain_news
  ADD CONSTRAINT PK_hms_mountain_news -- 산악뉴스 기본키
  PRIMARY KEY (
  mountain_news_id, -- 산악뉴스번호
  board_id          -- 게시판번호
  );

ALTER TABLE hms_mountain_news
  MODIFY COLUMN mountain_news_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '산악뉴스번호';
...
-- 산
ALTER TABLE hms_mountain_info
  ADD CONSTRAINT FK_hms_region_TO_hms_mountain_info -- 지역 -> 산
  FOREIGN KEY (
  region_id -- 지역번호
  )
...
-- 산악뉴스
ALTER TABLE hms_mountain_news
  ADD CONSTRAINT FK_hms_board_TO_hms_mountain_news -- 게시판 -> 산악뉴스
  FOREIGN KEY (
  board_id -- 게시판번호
  )
  REFERENCES hms_board ( -- 게시판
...
-- 등산일지
ALTER TABLE hms_diary
  ADD CONSTRAINT FK_hms_mountain_info_TO_hms_diary -- 산 -> 등산일지
  FOREIGN KEY (
  mountain_info_id -- 산번호
  )
  REFERENCES hms_mountain_info ( -- 산
  mountain_info_id -- 산번호
  );
...
-- 등산코스
ALTER TABLE hms_course_info
  ADD CONSTRAINT FK_hms_mountain_info_TO_hms_course_info -- 산 -> 등산코스
  FOREIGN KEY (
  mountain_info_id -- 산번호
  )
  REFERENCES hms_mountain_info ( -- 산
